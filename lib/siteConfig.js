export const siteConfig = {
  // ==========================
  // Business Information
  // ==========================
  businessName: "Legacy Barbers",

  description:
    "Professional barber services with simple online booking.",

  // ==========================
  // Branding
  // ==========================
  branding: {
    logo: "/logo.png",
    heroImage: "/image.png",
  },

  // ==========================
  // Homepage
  // ==========================
  homePage: {
    badge: "Legacy Barbers",
    button: "Book Appointment",

    features: [
      {
        icon: "✂️",
        title: "Clean Cuts",
        text: "Sharp styles, clean fades, and detailed finishes.",
      },
      {
        icon: "💈",
        title: "Professional Service",
        text: "Reliable barber services in a welcoming environment.",
      },
      {
        icon: "🕒",
        title: "Easy Booking",
        text: "Choose a date and reserve your appointment online.",
      },
      {
        icon: "⭐",
        title: "Premium Experience",
        text: "Quality service and attention to detail from start to finish.",
      },
    ],
  },

  // ==========================
  // Homepage Hero
  // ==========================
  hero: {
    badge: "Legacy Barbers",
    title: "Sharp Cuts. Clean Confidence.",
    description:
      "Professional barber services with simple online booking.",
    primaryButton: "Book Appointment",
    secondaryButton: "View Services",
  },

  // ==========================
  // Footer
  // ==========================
  footer: {
    tagline:
      "Premium cuts. Professional service. Built for confidence.",
    designerName: "Joshua Paddy",
  },

  // ==========================
  // Contact Information
  // ==========================
  contact: {
    phone: "(647) 000-0000",
    email: "legacybarbers@email.com",
    address: "123 Main Street\nHamilton, Ontario",
  },

  // ==========================
  // Social Media
  // ==========================
  socialLinks: {
    instagram: "https://instagram.com",
    instagramUsername: "@legacybarbers",
    facebook: "",
    tiktok: "",
  },

  // ==========================
  // Contact Page
  // ==========================
  contactPage: {
    title: "Contact Us",
    description:
      "Have a question about a service or appointment? Reach out and we will get back to you as soon as possible.",
    formTitle: "Send a Message",
    formDescription:
      "Fill out the form and we will respond as soon as possible.",
  },

  // ==========================
  // Services Page
  // ==========================
  servicesPage: {
    title: "Our Services",
    description:
      "Premium barber services designed to keep you looking sharp. Every appointment includes attention to detail and a relaxing experience.",

    whyChooseTitle: "Why Choose Legacy Barbers?",
    whyChooseDescription:
      "We combine modern barbering techniques with attention to detail, creating clean cuts, sharp fades, and precise beard work in a welcoming atmosphere. Every client leaves looking and feeling confident.",

    bookButton: "Book Now",
    bottomButton: "Book Your Appointment",

    benefits: [
      {
        id: "quality",
        icon: "⭐",
        title: "Top Quality",
        description:
          "Professional service using premium tools and products.",
      },
      {
        id: "experience",
        icon: "💈",
        title: "Experienced Barbers",
        description:
          "Precision fades, modern styles, and classic cuts.",
      },
      {
        id: "timing",
        icon: "🕒",
        title: "Always On Time",
        description:
          "Book easily and get in the chair without long waits.",
      },
    ],
  },

  // ==========================
  // Default Services
  // ==========================
  services: [
    {
      id: "skin-fade",
      name: "Skin Fade",
      price: 35,
      duration: "45 min",
      description:
        "A clean skin fade with precision blending and a styled finish.",
      icon: "✂️",
      active: true,
      order: 1,
    },
    {
      id: "haircut",
      name: "Haircut",
      price: 30,
      duration: "40 min",
      description:
        "Classic or modern haircut tailored to your style and face shape.",
      icon: "💈",
      active: true,
      order: 2,
    },
    {
      id: "haircut-beard",
      name: "Haircut & Beard",
      price: 45,
      duration: "60 min",
      description:
        "Fresh haircut with a sharp beard trim and lineup.",
      icon: "🧔",
      active: true,
      order: 3,
    },
    {
      id: "beard-trim",
      name: "Beard Trim",
      price: 20,
      duration: "20 min",
      description:
        "Clean beard shaping, lineup, and finishing touches.",
      icon: "🪒",
      active: true,
      order: 4,
    },
    {
      id: "kids-haircut",
      name: "Kids Haircut",
      price: 25,
      duration: "30 min",
      description:
        "Professional haircut for children under 12.",
      icon: "👦",
      active: true,
      order: 5,
    },
    {
      id: "line-up",
      name: "Line Up",
      price: 15,
      duration: "15 min",
      description:
        "Sharp edges and a crisp hairline to keep your cut looking fresh.",
      icon: "📏",
      active: true,
      order: 6,
    },
  ],

  // ==========================
  // Booking Page
  // ==========================
  bookingPage: {
  title: "Book an Appointment",

  description:
    "Choose your service, select an available date, and reserve your appointment.",

  currentMonthLabel: "Currently Booking",

  beforeBookingTitle: "Before You Book",

  tips: [
    {
      number: "01",
      title: "Choose Your Service",
      description:
        "Select the haircut or grooming service you'd like to book.",
    },
    {
      number: "02",
      title: "Choose a Date",
      description:
        "Pick an available day and time that works best for you.",
    },
    {
      number: "03",
      title: "Arrive On Time",
      description:
        "Please arrive 5–10 minutes early for your appointment.",
    },
  ],

  serviceTitle: "Choose a Service",
  dateTitle: "Choose a Date",
  timeTitle: "Choose a Time",
  detailsTitle: "Your Details",

  bookingFormTitle: "Reserve Your Appointment",

  bookingFormDescription:
    "Complete the form below to book your appointment for",

  submitButton: "Book Appointment",

  closedTitle: "Bookings Are Currently Closed",

  closedDescription:
    "We're not accepting appointments right now. Please check back when bookings reopen.",

  contactButton: "Contact Us",

  bottomTitle: "Questions Before Booking?",

  bottomDescription:
    "If you need help choosing a service or have any questions, feel free to contact us.",

  bottomButton: "Contact Us",
},

  // ==========================
  // Booking Settings
  // ==========================
  booking: {
    title: "Book an Appointment",
    description:
      "Choose your service, select an available date, and reserve your appointment.",
    appointmentDuration: 60,
    bookingsOpen: true,
  },

  // ==========================
  // Gallery Page
  // ==========================
  galleryPage: {
    title: "Our Work",
    description:
      "Take a look at some of our latest cuts, fades, beard work, and transformations.",
  },

  galleryImages: [
    {
      id: "skinfade",
      imageUrl: "/skinfade.png",
      title: "Skin Fade",
      description: "Clean fade with a sharp finish.",
      alt: "Clean skin fade haircut",
    },
    {
      id: "beard-lineup",
      imageUrl: "/beard-lineup.png",
      title: "Haircut & Beard",
      description:
        "Fresh cut paired with a detailed beard trim.",
      alt: "Haircut and beard trim",
    },
    {
      id: "classic-cut",
      imageUrl: "/classic-cut.png",
      title: "Classic Cut",
      description: "Timeless style with a modern touch.",
      alt: "Classic barber haircut",
    },
    {
      id: "sharp-lineup",
      imageUrl: "/sharp-lineup.png",
      title: "Sharp Line Up",
      description: "Crisp edges and perfect detail.",
      alt: "Sharp hairline lineup",
    },
    {
      id: "premium-grooming",
      imageUrl: "/premium-grooming.png",
      title: "Premium Grooming",
      description: "Professional grooming experience.",
      alt: "Professional barber grooming service",
    },
    {
      id: "shop-interior",
      imageUrl: "/shop-interior.png",
      title: "Shop Interior",
      description:
        "A clean, modern barbershop built for comfort.",
      alt: "Modern barbershop interior",
    },
  ],

  // ==========================
  // Bottom Call To Action
  // ==========================
  cta: {
    title: "Ready for a Fresh Cut?",
    description:
      "Reserve your appointment before the available spots fill up.",
    button: "Book Appointment",
  },

  // ==========================
  // SEO
  // ==========================
  seo: {
    title: "Legacy Barbers | Professional Barber Services",
    description:
      "Book professional barber services including haircuts, fades, beard trims, and lineups.",
    keywords: [
      "barber",
      "barbershop",
      "haircut",
      "skin fade",
      "beard trim",
      "Hamilton barber",
    ],
  },

  // ==========================
  // Business Hours
  // ==========================
  defaultBusinessHours: {
    0: {
      name: "Sunday",
      closed: true,
      open: "10:00",
      close: "17:00",
    },
    1: {
      name: "Monday",
      closed: false,
      open: "10:00",
      close: "17:00",
    },
    2: {
      name: "Tuesday",
      closed: false,
      open: "10:00",
      close: "17:00",
    },
    3: {
      name: "Wednesday",
      closed: false,
      open: "10:00",
      close: "17:00",
    },
    4: {
      name: "Thursday",
      closed: false,
      open: "10:00",
      close: "17:00",
    },
    5: {
      name: "Friday",
      closed: false,
      open: "10:00",
      close: "17:00",
    },
    6: {
      name: "Saturday",
      closed: false,
      open: "10:00",
      close: "17:00",
    },
  },
};